 ...
 */
export function MonstersList ({ monsters, className }: MonstersListProps): React.ReactNode {
  ...
  return (
    <section className={`w-full space-y-6 ${className ?? ''}`}>
      <header className='space-y-2'>
        <h2 className='text-2xl font-bold text-slate-900'>Vos créatures</h2>
        <p className='text-sm text-slate-600'>
          Découvrez et interagissez avec vos compagnons
        </p>
      </header>

      <div className='grid gap-6 sm:grid-cols-2 xl:grid-cols-3'>
        {monsters.map((monster) => {
          const cardKey = String(monster._id)
